...
`getBatchStyleKeys()` 返回所有目标图元样式字段的交集。`updateBatch()` 只接受该交集内的样式字段；任何非交集字段都会使整次操作无效，避免部分图元被修改。`visible`、`locked` 与 `zIndex` 是所有图元共有的字段，可直接批量更新，锁定图元同样接受——锁定只冻结几何拖动与删除，`removeBatch()` 对混合选择只移除未锁定项（详见 `drawing-interaction-hardening.md` 的 locked 语义）。

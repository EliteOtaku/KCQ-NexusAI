# BaoStock / TradingView（`Baostock-Tradingview-Connector`，原 `stockbao`）
...
workspace/
├── KLineChartQuant/                     # 本仓库
└── Baostock-Tradingview-Connector/      # BaoStock / TradingView 数据后端
```
...
前置：Python 3.12 + [uv](https://docs.astral.sh/uv/)。`Baostock-Tradingview-Connector` 与本仓库同级：

```bash
# 在本仓库根目录执行（等价于 cd ../Baostock-Tradingview-Connector && uv run python ./server.py）
pnpm connector baostock

# 或在 Baostock-Tradingview-Connector 目录手动启动
cd ../Baostock-Tradingview-Connector
uv sync

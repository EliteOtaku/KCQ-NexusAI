...
async function main() {
  ...
  // 上游 #202 locked 语义修订：锁定图元只接受 locked 字段（visible/style 等编辑被门控拒绝），
  // 显隐切换须在解锁态进行；锁定态删除同样被拒，删除前先解锁。
  ...
  await page.waitForTimeout(150)
  check('B3-03：显隐切换（visible=false）', (await nx(page, 'window.__nx.ctrl.drawings.peek()[0].visible')) === false)
  await page.click('.nx-side-panel__section--objects [title="显示"]')
  await page.waitForTimeout(150)
  check('B3-03：显隐恢复（visible=true）', (await nx(page, 'window.__nx.ctrl.drawings.peek()[0].visible')) === true)
  await page.click('.nx-side-panel__section--objects [title="锁定"]')
  await page.waitForTimeout(150)
  check('B3-03：锁定切换', (await nx(page, 'window.__nx.ctrl.drawings.peek()[0].locked')) === true)
  await page.click('.nx-side-panel__section--objects [title="解锁"]')
  await page.waitForTimeout(150)

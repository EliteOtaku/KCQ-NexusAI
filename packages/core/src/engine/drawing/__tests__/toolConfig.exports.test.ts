...
describe('toolConfig anchor tables public exports', () => {
  it('getAnchorCountForTool 按锚点数分组返回 1/2/3，非绘图工具返回 null', () => {
    expect(SINGLE_ANCHOR_TOOLS.map(getAnchorCountForTool)).toEqual(
      SINGLE_ANCHOR_TOOLS.map(() => 1),
    )
    expect(DOUBLE_ANCHOR_TOOLS.map(getAnchorCountForTool)).toEqual(
      DOUBLE_ANCHOR_TOOLS.map(() => 2),
    )
    expect(TRIPLE_ANCHOR_TOOLS.map(getAnchorCountForTool)).toEqual(
      TRIPLE_ANCHOR_TOOLS.map(() => 3),
    )

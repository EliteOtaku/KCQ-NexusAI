...
- [x] 上游同步（2026-09-18 晚）：upstream/main e4b6fdfe→25c31e3c（#202 drawing-drag-policy 等 24+ 提交）已 merge 进 nexus/main=5425e24a 并 push origin；core 2591 全绿/nexus-shell typecheck 绿/topbar 15/15；**探针适配待办**：probe-drawing B1-04 三锚点通道创建 FAIL（上游改通道右端点由光标驱动）、probe-b234 B3-03 显隐切换 FAIL+首轮超时（locked 语义修订为"可选中不可拖"）——需按上游新交互模型适配探针/壳侧

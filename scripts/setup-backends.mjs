...
const BACKENDS = [
  ...
  },
  {
    ...
    purpose:
      'MT5 本地终端行情后端（Exness），FastAPI 默认端口 8090，`pnpm connector mt5` 需要（Windows + 已登录 MT5 终端）',
  },
]
...
console.log('完成。启动命令：')
console.log('  pnpm dev -c all               # Vite 开发服务器 + 全部 connector（不含 mt5）')
console.log('  pnpm dev -c gotdx baostock    # 前端 + 指定的 connector')
console.log('  pnpm connector baostock       # 仅 BaoStock / TradingView 后端')
console.log('  pnpm connector mt5            # 仅 MT5 本地终端后端（Windows + 已登录 MT5 终端）')
console.log(
